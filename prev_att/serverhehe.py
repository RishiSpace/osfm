...
class Server(QtWidgets.QMainWindow):
    ...
    def __init__(self, host="0.0.0.0", port=12345):
        ...
        self.connections = {}
        self.network_share_path = r"\\NETWORK_SHARE\path\to\folder"  # Set the network share path
        self.setup_ui()
    ...
    def install_selected_software(self):
        for name in self.selected_software:
            ...
            for hostname in self.connections:
                self.send_download_path(pkg_id, hostname)
    ...
    def send_download_path(self, pkg_id, host, hostname):
        ...
        if file_path:
            command = f"FILE_PATH {host}\\{file_path}"
            if host in self.connections:
                try:
                    self.connections[host].sendall(command.encode())
                    print(f"Sent file path to {hostname}: {file_path}")
                except Exception as e:
