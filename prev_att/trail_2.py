            ...
            if result.returncode != 0:
                ...
                for line in result.stdout.splitlines():
                    if 'Downloaded:' in line:
                        file_name = line.split('Downloaded:')[1].strip()
                        return file_name
                return None
        except Exception as e:
    ...
    def send_downloaded_software(self):
        for conn in self.connections.values():
            try:
                for name in self.selected_software:
                    pkg_id = self.software_options[name]
                    file_name = self.download_software(pkg_id)
                    if file_name:
                        file_path = os.path.join("temp", file_name)
                        with open(file_path, "rb") as file:
                            conn.sendall(f"UPLOAD {file_name}".encode())
...
if __name__ == "__main__":
    ...
    server = Server()
    server.create_install_software_gui()
    sys.exit(app.exec_())